...
app.get("/search", bodypars, function(req,res){
    ...
    requesting.get("http://127.0.0.1:8000/search", {form: {nameDrug: req.query.nameDrug}}, function(err,resp,body){
        if(resp.statusCode == 200){
            var data = JSON.parse(body); 
            res.render('search',{isLogged: isLogged, dataNeyron: data.Neyron[0], nameDrug: nameDrug, dataAptekaRU: data.AptekaRU[0], 
                dataZhivika: data.Zhivika[0], dataAptekaOtSklada: data.AptekaOtSklada[0], dataEapteka: data.Eapteka[0],
